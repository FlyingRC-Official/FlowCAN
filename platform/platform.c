...
static volatile bool pmw_irq,tof_irq;
...
static void init_can(void)
{
    ...
    can_baudrate_type baud={.baudrate_div=4U,.rsaw_size=CAN_RSAW_3TQ,.bts1_size=CAN_BTS1_13TQ,.bts2_size=CAN_BTS2_4TQ};(void)can_baudrate_set(CAN1,&baud);
    ...
    can_interrupt_enable(CAN1,CAN_RF0MIEN_INT,TRUE);can_interrupt_enable(CAN1,CAN_ETRIEN_INT,TRUE);can_interrupt_enable(CAN1,CAN_EOIEN_INT,TRUE);nvic_irq_enable(CAN1_RX0_IRQn,3U,0U);nvic_irq_enable(CAN1_SE_IRQn,3U,0U);
}
static void init_timers(void)
{
    ...
    crm_periph_clock_enable(CRM_DMA1_PERIPH_CLOCK,TRUE);dma_flexible_config(DMA1,FLEX_CHANNEL2,DMA_FLEXIBLE_TMR1_OVERFLOW);dma_init_type d;dma_default_para_init(&d);d.buffer_size=WS_BITS;d.direction=DMA_DIR_MEMORY_TO_PERIPHERAL;d.memory_base_addr=(uint32_t)ws_dma;d.memory_data_width=DMA_MEMORY_DATA_WIDTH_HALFWORD;d.memory_inc_enable=TRUE;d.peripheral_base_addr=(uint32_t)&TMR1->c1dt;d.peripheral_data_width=DMA_PERIPHERAL_DATA_WIDTH_HALFWORD;d.peripheral_inc_enable=FALSE;d.priority=DMA_PRIORITY_MEDIUM;d.loop_mode_enable=FALSE;dma_init(DMA1_CHANNEL2,&d);dma_interrupt_enable(DMA1_CHANNEL2,DMA_FDT_INT,TRUE);tmr_dma_request_enable(TMR1,TMR_OVERFLOW_DMA_REQUEST,TRUE);nvic_irq_enable(DMA1_Channel2_IRQn,6U,0U);
}

void platform_init(void)
{
    nvic_priority_group_config(NVIC_PRIORITY_GROUP_4);init_gpio();init_exti();init_spi();init_i2c();init_uart();init_can();init_timers();SysTick_Config(SYSCLK_HZ/SYSTICK_HZ);
    wdt_register_write_enable(TRUE);wdt_divider_set(WDT_CLK_DIV_256);wdt_reload_value_set(78U);wdt_counter_reload();wdt_enable();
}
uint32_t platform_millis(void){return milliseconds;}uint32_t platform_micros(void){return tmr_counter_value_get(TMR2);}void platform_delay_us(uint32_t us){uint32_t s=platform_micros();while((uint32_t)(platform_micros()-s)<us){}}
void platform_watchdog_feed(void){wdt_counter_reload();}
uint8_t platform_spi_transfer(uint8_t v){while(spi_i2s_flag_get(SPI1,SPI_I2S_TDBE_FLAG)==RESET){}spi_i2s_data_transmit(SPI1,v);while(spi_i2s_flag_get(SPI1,SPI_I2S_RDBF_FLAG)==RESET){}return(uint8_t)spi_i2s_data_receive(SPI1);}
void platform_pmw_cs(bool a){gpio_bits_write(GPIOA,GPIO_PINS_4,a?FALSE:TRUE);}void platform_pmw_reset(bool a){gpio_bits_write(GPIOB,GPIO_PINS_0,a?FALSE:TRUE);}bool platform_take_pmw_irq(void){bool v=pmw_irq;pmw_irq=false;return v;}
void platform_tof_xshut(bool e){gpio_bits_write(GPIOB,GPIO_PINS_5,e?TRUE:FALSE);}bool platform_take_tof_irq(void){bool v=tof_irq;tof_irq=false;return v;}

static bool wait_i2c(uint32_t flag,uint32_t timeout){uint32_t s=platform_micros();while(i2c_flag_get(I2C1,flag)==RESET){if((uint32_t)(platform_micros()-s)>=timeout)return false;}return true;}
bool platform_i2c_write(uint8_t addr,const uint8_t *data,size_t len,uint32_t timeout)
{
    uint32_t s=platform_micros();while(i2c_flag_get(I2C1,I2C_BUSYF_FLAG)!=RESET){if((uint32_t)(platform_micros()-s)>=timeout)return false;}i2c_start_generate(I2C1);if(!wait_i2c(I2C_STARTF_FLAG,timeout))return false;i2c_7bit_address_send(I2C1,addr,I2C_DIRECTION_TRANSMIT);if(!wait_i2c(I2C_ADDR7F_FLAG,timeout))return false;i2c_flag_clear(I2C1,I2C_ADDR7F_FLAG);
    for(size_t i=0;i<len;i++){if(!wait_i2c(I2C_TDBE_FLAG,timeout))return false;i2c_data_send(I2C1,data[i]);}if(!wait_i2c(I2C_TDBE_FLAG,timeout))return false;i2c_stop_generate(I2C1);return true;
}
bool platform_i2c_read(uint8_t addr,uint8_t *data,size_t len,uint32_t timeout)
{
    if(len==0U)return true;
    i2c_ack_enable(I2C1,TRUE);i2c_start_generate(I2C1);if(!wait_i2c(I2C_STARTF_FLAG,timeout))return false;i2c_7bit_address_send(I2C1,addr,I2C_DIRECTION_RECEIVE);if(!wait_i2c(I2C_ADDR7F_FLAG,timeout))return false;i2c_flag_clear(I2C1,I2C_ADDR7F_FLAG);
    for(size_t i=0;i<len;i++){if(i+1U==len)i2c_ack_enable(I2C1,FALSE);if(!wait_i2c(I2C_RDBF_FLAG,timeout))return false;data[i]=i2c_data_receive(I2C1);}i2c_stop_generate(I2C1);i2c_ack_enable(I2C1,TRUE);return true;
}
bool platform_uart_tx(const uint8_t *data,size_t len){for(size_t i=0;i<len;i++){uint16_t n=(uint16_t)((utx_head+1U)%UART_RING_SIZE);if(n==utx_tail)return false;uart_tx[utx_head]=data[i];utx_head=n;}usart_interrupt_enable(USART1,USART_TDBE_INT,TRUE);return true;}
...
bool platform_can_tx(const CanardCANFrame *f){can_tx_message_type t={0};t.id_type=(f->id&CANARD_CAN_FRAME_EFF)?CAN_ID_EXTENDED:CAN_ID_STANDARD;t.extended_id=f->id&CANARD_CAN_EXT_ID_MASK;t.standard_id=f->id&CANARD_CAN_STD_ID_MASK;t.frame_type=CAN_TFT_DATA;t.dlc=f->data_len;memcpy(t.data,f->data,f->data_len);return can_message_transmit(CAN1,&t)!=CAN_TX_STATUS_NO_EMPTY;}
...
bool platform_can_bus_off(void){return can_flag_get(CAN1,CAN_BOF_FLAG)==SET;}void platform_can_reinit(void){can_reset(CAN1);init_can();}
bool platform_take_can_rx_overflow(void){bool v=can_overflow;can_overflow=false;return v;}
bool platform_take_can_rx_activity(void){bool v=can_activity;can_activity=false;return v;}
...
void EXINT1_IRQHandler(void){if(exint_flag_get(EXINT_LINE_1)!=RESET){exint_flag_clear(EXINT_LINE_1);pmw_irq=true;}}
void EXINT9_5_IRQHandler(void){if(exint_flag_get(EXINT_LINE_8)!=RESET){exint_flag_clear(EXINT_LINE_8);tof_irq=true;}}
void USART1_IRQHandler(void)
{
    if(usart_flag_get(USART1,USART_RDBF_FLAG)!=RESET){uint16_t n=(uint16_t)((urx_head+1U)%UART_RING_SIZE);uint8_t b=(uint8_t)usart_data_receive(USART1);if(n!=urx_tail){uart_rx[urx_head]=b;urx_head=n;}}
...
{
    can_rx_message_type r;while(CAN1->rf0_bit.rf0mn!=0U){can_message_receive(CAN1,CAN_RX_FIFO0,&r);can_activity=true;uint8_t n=(uint8_t)((can_head+1U)%CAN_RING_SIZE);if(n==can_tail){can_overflow=true;continue;}can_slot_t *s=(can_slot_t *)&can_rx[can_head];s->frame.id=(r.id_type==CAN_ID_EXTENDED?r.extended_id|CANARD_CAN_FRAME_EFF:r.standard_id);s->frame.data_len=r.dlc;memcpy(s->frame.data,r.data,r.dlc);s->timestamp=platform_micros();can_head=n;}
}
void CAN1_SE_IRQHandler(void){can_flag_clear(CAN1,CAN_EOIF_FLAG);}
