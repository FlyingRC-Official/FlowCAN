...
void platform_init(void)
{
    ...
    init_can();
#endif
    init_timers();SysTick_Config(SYSCLK_HZ/SYSTICK_HZ);
...
void platform_pmw_cs(bool a){gpio_bits_write(GPIOA,GPIO_PINS_4,a?FALSE:TRUE);}void platform_pmw_reset(bool a){gpio_bits_write(GPIOB,GPIO_PINS_0,a?FALSE:TRUE);}bool platform_take_pmw_irq(void){return take_flag(&pmw_irq);}
void platform_tof_xshut(bool e){gpio_bits_write(GPIOB,GPIO_PINS_5,e?TRUE:FALSE);}bool platform_take_tof_irq(void){return take_flag(&tof_irq);}
...
bool platform_uart_tx(const uint8_t *data,size_t len){const uint16_t head=utx_head,tail=utx_tail;if(data==NULL&&len!=0U)return false;if(!ring_can_write(UART_RING_SIZE,head,tail,len))return false;uint16_t next=head;for(size_t i=0;i<len;i++){uart_tx[next]=data[i];next=(uint16_t)((next+1U)%UART_RING_SIZE);}utx_head=next;usart_interrupt_enable(USART1,USART_TDBE_INT,TRUE);return true;}
