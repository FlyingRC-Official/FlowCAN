...
{
    if (count>252U || (count!=0U && data==NULL)) return -1;
...
int8_t VL53L1_RdWord(uint16_t d,uint16_t r,uint16_t *v){if(v==NULL)return -1;uint8_t b[2]={0U,0U};int8_t e=VL53L1_ReadMulti(d,r,b,2U);if(e==0)*v=(uint16_t)((uint16_t)b[0]<<8U|b[1]);return e;}
int8_t VL53L1_RdDWord(uint16_t d,uint16_t r,uint32_t *v){if(v==NULL)return -1;uint8_t b[4]={0U,0U,0U,0U};int8_t e=VL53L1_ReadMulti(d,r,b,4U);if(e==0)*v=(uint32_t)b[0]<<24U|(uint32_t)b[1]<<16U|(uint32_t)b[2]<<8U|b[3];return e;}
