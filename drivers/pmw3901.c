...
static bool transfer(uint8_t value, uint8_t *received)
{
    return platform_spi_transfer(value, received);
}

static bool write_reg(uint8_t reg, uint8_t value)
{
    uint8_t ignored;
    platform_pmw_cs(true);
    platform_delay_us(50U);
    const bool ok = transfer((uint8_t)(reg | 0x80U), &ignored) && transfer(value, &ignored);
    platform_delay_us(50U);
    platform_pmw_cs(false);
    platform_delay_us(200U);
    return ok;
}
...
static bool read_reg(uint8_t reg, uint8_t *value)
{
    ...
    platform_pmw_cs(true);
    platform_delay_us(50U);
    const bool address_ok = transfer((uint8_t)(reg & 0x7FU), &ignored);
    platform_delay_us(50U);
    const bool data_ok = address_ok && transfer(0U, value);
    platform_delay_us(100U);
    platform_pmw_cs(false);
    return data_ok;
}
...
pmw_init_result_t pmw3901_init_step(pmw3901_t *d, uint32_t now)
{
    ...
    if(d->init_state==INIT_RELEASE_WAIT){if(!write_reg(0x3A,0x5A)){d->init_state=INIT_FAILED;return PMW_INIT_FAULT;}d->init_state=INIT_ID_WAIT;d->init_deadline_us=now+5000U;return PMW_INIT_BUSY;}
    if(d->init_state==INIT_ID_WAIT){uint8_t product=0U,inverse=0U;if(!read_reg(0x00,&product)||!read_reg(0x5F,&inverse)||product!=PMW3901_PRODUCT_ID||inverse!=PMW3901_INVERSE_ID){d->init_state=INIT_FAILED;return PMW_INIT_FAULT;}for(uint8_t r=0x02;r<=0x06;r++){uint8_t ignored;if(!read_reg(r,&ignored)){d->init_state=INIT_FAILED;return PMW_INIT_FAULT;}}d->init_state=INIT_SEQUENCE;}
    if(d->init_state==INIT_SEQUENCE){const size_t count=sizeof(init_sequence)/sizeof(init_sequence[0]);if(d->init_index<count){const reg_init_t *entry=&init_sequence[d->init_index++];if(!write_reg(entry->reg,entry->value)){d->init_state=INIT_FAILED;return PMW_INIT_FAULT;}d->init_deadline_us=now+(uint32_t)entry->delay_ms*1000U;return PMW_INIT_BUSY;}d->initialized=true;d->integration_start_us=now;d->init_state=INIT_DONE;return PMW_INIT_READY;}
...
bool pmw3901_read_motion(pmw_motion_t *m)
{
    ...
    uint8_t ignored;
    bool ok=true;
    platform_pmw_cs(true); platform_delay_us(35U); ok=transfer(0x16U,&ignored);
    platform_delay_us(35U); for (size_t i=0;i<sizeof(b)&&ok;i++) ok=transfer(0U,&b[i]); platform_delay_us(100U); platform_pmw_cs(false);
    if(!ok)return false;
    bool all_one=true;
    for(size_t i=0;i<sizeof(b);i++)all_one=all_one&&b[i]==0xFFU;
...
{
    uint8_t product=0U,inverse=0U;
    return read_reg(0x00,&product)&&read_reg(0x5F,&inverse)&&
           product==PMW3901_PRODUCT_ID&&inverse==PMW3901_INVERSE_ID;
}
